 ...
 */
/**
 * 本程序是客户端程序或者后台程序模拟浏览器的upload对数据进行装箱操作
 * . 具体协议请参考w3c RFC1867规范 有任何意见请到这里与我交流:
 * 
...
public class MultiPartDataSender {
		...
		try {
			...
			for (Iterator<String> it = this.textFields.keySet().iterator(); it
				...
				content.append("Content-Disposition: form-data; name=\"")
						.append(name).append("\"\r\n");
				content.append("\r\n");
				content.append(textFields.get(name)).append("\r\n");
			}
			...
			for (String file : localFiles) {
				...
				content.append("Content-Disposition: form-data; name=\"file_"
						+ p++ + "\"; filename=\"" + file + "\"\r\n");
				content.append("Content-Type: " + getContentType(file)).append(
						"\r\n");
				content.append("\r\n");
				tmpFout.write(content.toString().getBytes());
				tmpFout.flush();
			...
			header
					.append("Accept-Encoding: deflate, gzip, x-gzip, identity, *;q=0\r\n");
			...
			if (cookies.size() > 0) {
				...
				for (Iterator<String> it = cookies.keySet().iterator(); it
						.hasNext();) {
					cookiename = it.next();
					header.append(cookiename).append("=").append(
	...
	private static String getContentType(String file) {
		try {
			...
			return furl.openConnection().getContentType();
		} catch (Exception e) {

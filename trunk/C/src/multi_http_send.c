...
int  http_multi_send(struct st_multi_req__ *req)
{
  ...
  //http:// len =7
    //find first '/'  file.
   tmp_p=strchr(req->url+7,'/');
   if(tmp_p)
    {//file !='/'
       bzero(host_port,url_len);
       strncpy(host_port,req->url+7,strlen(req->url)-strlen(tmp_p)-7);
       strcpy(url_file,tmp_p);
    }else{
         strcpy(host_port,req->url+7);
    }
 //find port
  ...
  //open connection...
   ...
   // serv_addr.sin_addr=*((struct in_addr *)server->h_addr);
    ...
    for(loop_p=0,ck_p=req->files;ck_p<req->files+req->file_count;ck_p++,loop_p++)
    {
       ...
       fprintf(tmp_fp,"Content-Type:application/octet-stream\r\n");
       fprintf(tmp_fp,"\r\n");
	...
	sprintf(http_header,header_str,url_file,host,bound_id+2,ftell(tmp_fp));
	fclose(tmp_fp);
